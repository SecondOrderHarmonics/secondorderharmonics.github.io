...
layout: home
title: Home
landing-title: "Analogue Sound for the Digital Age"
description: "Hand-crafted and artisanal Audio/Studio exquisites from new products to modded clones of only the best consoles/parts from the 50ies to late 80ies. Design, Manufacture, Customization, Repair & Improvement"
...
1. Customized Clones of Neve, API, EMI, and other famous audio consoles
   ...
   brands)
2. Summing: In todays world everyone uses a DAW for recording, even
   Studios with 96 Channel Consoles have ProTools running,
   audio-interfaces with fairly decent specs have become affordable for
   musicians themselves, as have some outboard solutions: Pres, EQs, FX.
    - Without summing and bussing to various 2TRK (two track) outputs,
    it's fairly difficult to work with low-latency outboard gear while
    editing mixes in a DAW or tracking live via a ADC/DAC interface and
    controling it's inputs/faders/impendence settings (e.g. HiZ).
    - Most summimng units on the market are hugely overprized, we have
    no idea why. One of the first units we came up with was a 8 Channel
    to 2xCombo XLR/TRS (Stereo or Mono) outputs that fits in one hand.
    The design has since been open-sourced and the hardware is still
    being updated with newer revisions from time to time. But customers
    are usually asking us for specific requirements - everyone has other
    outboard gear, different audio cards or ADC/DACs from UAD to Lavry,
    from MOTU to Focusrite to Arturia, there are so many on the market,
    you can spend hours just on finding an appropriate ADAT S/MUX
    expander. Summing helps. For some Outputs but especially for Inputs.
    Just like having an external 8-12CH mixer routed to the input of
    your ADC/Audio Interface you can sum the channels of different
    inputs to 2TRK/Main on a Summing Bus. And we feel you shouldn't pay
    thousands of euros for - often passive - products in a nice case.
    - So..customized summers: Want to amplifiy Channel 1-8 but
    not 8-16? Because 1-8 are direct inputs whereas 8-16 come from
    dedicated pre-amps or a mixer? No Problem. Want multiple busses,
    inserts, gain staging, output OpAmps or Transformers? Sure. Talk to
    us.
3.  Amplification: from Mic Pre-amps to Console/Outboard Bus Amps. We
    work on all of it: clones of famous vintage equipment, our own
    designs, mods of existing equipment (old and new, vintage and
    clones). We upgrade to the newest IC OpAmps on the Market with THD+N
